...
__global__ void row_unpack_8bit(
  ...
  int tmp_val;
  int tmp_sign;
  val = __float2int_rn(scale * inp[L_idx * D + T_idx]);

  for (int jdx = 0; jdx < count; jdx++) {
    tmp_val = val % INT8S;
    tmp_sign = tmp_val < 0;
    int8_t output_val = (tmp_sign << (INT8B - 1)) | (tmp_val & 127);
    out[(offset + jdx) * T + T_idx] = output_val;
    val = val / INT8S;
  }
}
...
at::Tensor row_unpack_launch(at::Tensor inp, at::Tensor cum_cnt, float scale, int X, int bits) {
  ...
  if (bits == 8) {
    int T = D;
    at::Tensor out = at::empty({X, T}, cum_cnt.options().dtype(at::kChar));
